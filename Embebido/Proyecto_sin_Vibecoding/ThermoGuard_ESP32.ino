...
// Texto de la línea 0 del LCD (modo actual)
char modoDisplay[17] = "ThermoGuard";

// Texto de la línea 1 del LCD para estado Alarm/IDLE
...
const char* topic_set_caliente = "grupoM3/set/tempCaliente";
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  // ---- Configuracion de setpoints (la app manda el valor en °C como texto) ----
  else if (topicStr == topic_set_frio)
  {
    float v = mensaje.toFloat();
    xSemaphoreTake(mutexDatos, portMAX_DELAY);
    defTempLow = v;
    xSemaphoreGive(mutexDatos);
    Serial.println("defTempLow actualizado -> " + String(v));
  }
  else if (topicStr == topic_set_medio)
  {
    float v = mensaje.toFloat();
    xSemaphoreTake(mutexDatos, portMAX_DELAY);
    defTempMedium = v;
    xSemaphoreGive(mutexDatos);
    Serial.println("defTempMedium actualizado -> " + String(v));
  }
  else if (topicStr == topic_set_caliente)
  {
    float v = mensaje.toFloat();
    xSemaphoreTake(mutexDatos, portMAX_DELAY);
    defTempHigh = v;
    xSemaphoreGive(mutexDatos);
    Serial.println("defTempHigh actualizado -> " + String(v));
  }
}
...
void reconectarMQTT() {
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      client.subscribe(topic_set_frio);
      client.subscribe(topic_set_medio);
      client.subscribe(topic_set_caliente);
    } else {
...
void taskMQTT(void *pvParameter) {
  ...
  while(1) {
    ...
    vTaskDelay(pdMS_TO_TICKS(50)); // Pequeño delay para no bloquear RTOS
  }
}
